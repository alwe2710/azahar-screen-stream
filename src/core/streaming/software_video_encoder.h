...
// (same libx264/libx265 wrapper, same interface) -- see that file's own
// comments for the reasoning behind the rate-control/profile/keyframe
// choices baked into the constructor, none of which are Azahar-specific.
...
public:
    ...
    // (see m_codedWidth's own comment; N3DS_BOTTOM_SCREEN's fixed 320x240 is
    // already 16-aligned on both axes, so in practice this always equals
    // width/height here, but SendVideoFrame() should still send this value
    // rather than assuming so). SendVideoFrame() must send THIS, not the
...
private:
    ...
    // includer; software_video_encoder.cpp is the only translation unit
    // that needs the real encoder types.
